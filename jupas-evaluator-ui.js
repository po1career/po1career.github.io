...
(function () {
  ...
  var STR = {
    en: {
      ...
      choicesHint: 'Position shows the computed score against this programme’s past-year quartiles (score only). Chance is based on the score position AND which exact choice slot it’s placed in — choices #1–#3 (Band A) each get progressively stricter thresholds, #4 onward only counts a score at/above the upper quartile as a real shot. Band flags whether the placement matches how Band-A-dependent the programme’s real offer history is — this is where the historical offer-rate signal lives now. Eligible checks minimum subject/grade requirements, including CSD and any programme-specific extra entry rules. Intake is the programme’s published first-year quota.',
    ...
    zh: {
      ...
      choicesHint: '「位置」只反映計算分數對比該課程過往年度四分位數（純分數比較）。「機會」則根據分數位置，以及該選項置於第幾個志願而定——第 1 至 3 志願（Band A）各有遞增的門檻，第 4 個志願起則只有達到或高於上四分位數的分數才視為有實質機會。「Band」標示所置放的志願組別是否配合該課程實際取錄歷史對 Band A 的依賴程度——過往取錄率的訊號現在集中在此欄。「資格」檢查是否符合最低科目／成績要求，包括公民與社會發展科及任何課程的額外收生要求。「學額」為課程公布的首年學額。',
